...
from qai_hub_models.models._shared.deepseekr1.split_onnx_utils import utils
...
import torch
def export_model(
    model_cls: type[DeepSeekR1Base_Quantizable],
    model_name: str,
    components: list[str],
    ...
    target_runtime: TargetRuntime = TargetRuntime.QNN, # 做Quantize以后导出QNN格式模型文件
    compile_options: str = "",
    profile_options: str = "",
    ...
    **additional_model_kwargs,
    ...
    """
    In this workflow, two instantiations of the DeepSeek model are exported (AR-1,
    ...
    for instantiation_name, seq_len in instantiations:
        ...
        source_model = model.convert_to_hub_source_model( # 把hugging face 的pytorch模型转成ONNX格式
            target_runtime,
            sub_output_path,
            input_spec,
        ...
        if source_model_path.suffix == ".pt":
            # 上传到服务器做编译，生成可以在手机上跑的模型，上传1G文件，网速不高，需要等30分钟左右。
            component_name = f"full"
            sub_component_names[instantiation_name].append(component_name)
            ...
            submitted_compile_job = hub.submit_compile_job(
                ...
                name=full_name,
                options="--target_runtime qnn_lib_aarch64_android --quantize_full_type w8a16",
                retry=True,
                single_compile=True
            )
